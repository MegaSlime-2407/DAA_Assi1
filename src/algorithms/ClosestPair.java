...
public class ClosestPair {
    ...
    public static Result findClosestPair(Point[] points, Metrics m) {
        if (m != null) m.reset();
        if (points == null || points.length < 2) throw new IllegalArgumentException("Need at least 2 points");
        Point[] px = points.clone();
        Point[] py = points.clone();
        sortByX(px);
        sortByY(py);
        return closestPair(px, py, 0, px.length - 1, m);
    }
    ...
    private static Result closestPair(Point[] px, Point[] py, int left, int right, Metrics m) {
        if (m != null) m.enter();
        Result out;
        int n = right - left + 1;
        if (n <= 3) out = bruteForceM(px, left, right, m);
        else {
            int mid = (left + right) / 2;
            Point midPoint = px[mid];
            Point[] pyl = new Point[mid - left + 1];
            ...
            int li = 0, ri = 0;
            for (Point p : py) {
                if (p != null && p.x <= midPoint.x && li < pyl.length) pyl[li++] = p; else if (p != null && ri < pyr.length) pyr[ri++] = p;
            }
            Result dl = closestPair(px, pyl, left, mid, m);
            Result dr = closestPair(px, pyr, mid + 1, right, m);
            Result d = dl.distance < dr.distance ? dl : dr;
            if (m != null) m.stripChecks++;
            Point[] strip = new Point[n];
            int j = 0;
            for (Point p : py) if (p != null && Math.abs(p.x - midPoint.x) < d.distance) strip[j++] = p;
            out = stripClosestM(strip, j, d, m);
        }
        if (m != null) m.leave();
        return out;
    }
    ...
    private static Result bruteForceM(Point[] points, int left, int right, Metrics m) {
        double min = Double.MAX_VALUE; Point p1 = null, p2 = null;
        for (int i = left; i <= right; i++) {
            for (int j = i + 1; j <= right; j++) {
    ...
    private static Result stripClosestM(Point[] strip, int size, Result d, Metrics m) {
        double min = d.distance; Point p1 = d.p1, p2 = d.p2;
        for (int i = 0; i < size; i++) {
            for (int j = i + 1; j < size && (strip[j].y - strip[i].y) < min; j++) {
                if (m != null) m.comparisons++;
                double dist = distance(strip[i], strip[j]);
